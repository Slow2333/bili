...
if [ -f "$(dirname "$0")/utils_env.sh" ]; then
	...
	wget -q -O utils_env.sh https://pd.zwc365.com/https://raw.githubusercontent.com/Oreomeow/checkinpanel/master/utils_env.sh
	source "$(dirname "$0")/utils_env.sh"
fi
get_some_path
...
download() {
	curl -L -o "./BILIBILI-HELPER.zip" "https://pd.zwc365.com/$download_url"
	mkdir ./tmp
	echo "正在解压文件......."
